...
export default {
  ...
  methods: {
    form_submit() {//表单提交事件
      ...
      _.$http.post('/index.php/Api/SecondHand/sellingCar', submit_data)
    ...
    carbrand_check(key, is_datatime = false) {
      ...
      new Promise((resolve, reject) => {
        ...
        _.CarUsedData[key] = ''
        is_datatime && (
          document.querySelector('.vux-datetime-value').className = document.querySelector('.vux-datetime-value').className.replace('have-value','')
        )
      })
      return {valid: false}
    },
    ...
    date_change(newVal) {
      ...
      if (newVal != '' && date_dom.className.match(/[^(have\-value)]/g)) {
        date_dom.className += ' have-value'
      }
    },
		address_show() {
